...
// se crea la funcion que lee con timeout 
package telnet
...
// Read reads all data into string from telnet device until it meets the expected or stops on timeout.
func (t Telnet) Read_con_tiempo(expect string , tiempo int) (str string, err error) {
	var buf bytes.Buffer
	t.timeout = time.Second * time.Duration(tiempo) // default
	t.conn.SetReadDeadline(time.Now().Add(t.timeout))
